...
@Data
@Table(name = "servico")
public class Servico implements BaseDomain<UUID> {
    ...
    @Column(name = "disponivel_agendamento")
    private boolean disponivelAgendamento = false;

    @Column(name = "disponivel_pacote")
    private boolean disponivelPacote = true;

    @NotNull
    private boolean ativo = true;

    @NotEmpty(message = "Informe pelo menos um tipo de veículo que o serviço atende!")
    @OneToMany(cascade = CascadeType.ALL, orphanRemoval = true)
    @JoinColumn(name = "id_servico", nullable = false)
    private List<ServicoTipoVeiculo> tiposVeiculos = new ArrayList<>();
    ...
    public boolean isValorVariavel() {
        return valor.equals(BigDecimal.ZERO);
    }
